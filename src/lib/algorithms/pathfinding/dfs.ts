...
export const dfs = (grid: GridType, startTile: TileType, endTile: TileType) => {
  ...
  while (untraversedTiles.length > 0) {
    ...
    const currentTile = untraversedTiles.pop(); // Get the last tile from the stack //* This is the main different between BFS
    ...
    for (let i = 0; i < neighbors.length; i += 1) {
      ...
      if (checkStack(neighbors[i], untraversedTiles)) continue; // Check if the neighbor is not in the stack
