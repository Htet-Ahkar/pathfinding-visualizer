...
export const initHeuristicCost = (grid: GridType, endTile: TileType) => {
  const heuristicCost: number[][] = []; // Initialize an empty array to store heuristic costs
  for (let i = 0; i < MAX_ROWS; i += 1) {
    // Loop through each row in the grid
    const row: number[] = []; // Initialize an empty array to store heuristic costs for the current row
...
export const initFunctionCost = () => {
  const functionCost: number[][] = []; // Initialize an empty array to store function costs
  for (let i = 0; i < MAX_ROWS; i += 1) {
    // Loop through each row in the grid
    const row: number[] = []; // Initialize an empty array to store function costs for the current row
    for (let j = 0; j < MAX_COLS; j += 1) {
      // Loop through each column in the current row
